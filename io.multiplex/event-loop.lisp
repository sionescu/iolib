        ...
        (destructuring-bind (fd ev-types) ev
          (let* ((fd-entry (fd-entry-of event-base fd))
            ...
            (labels ((append-events (events)
                     ...
                     (call-error-handlers ()
                       (%dispatch-event fd-entry :error errorp now)
                       (append-events (fd-entry-all-events fd-entry)))
                     (call-read-handlers ()
                       (let ((events (%dispatch-event fd-entry :read errorp now)))
                         (or errorp (append-events events))))
                     (call-write-handlers ()
                       (let ((events (%dispatch-event fd-entry :write errorp now)))
                         (or errorp (append-events events)))))
              (cond
                (fd-entry
                 (when (member :read ev-types) (call-read-handlers))
                 (when (member :write ev-types) (call-write-handlers))
                 (when errorp (call-error-handlers)))
                (t
...
(defun %dispatch-event (fd-entry event-type errorp now)
  (let ((deletion-list ())
    ...
    (funcall (fd-event-handler ev)
             (fd-entry-fd fd-entry) event-type errorp)
