...
(define-condition gpg-error (error)
  ((source :initarg :source :reader gpg-error-source)
   (code :initarg :code :reader gpg-error-code))
  (:report (lambda (condition stream)
             (let ((err (gpg-err-make
                         (gpg-error-source condition)
                         (gpg-error-code condition))))
               (format stream "~A caused error: ~S"
                       (gpg-strsource err)
                       (gpg-strerror err))))))

(defun signal-gpg-error (err)
  (error 'gpg-error
         :source (gpg-err-source err)
         :code (gpg-err-code err)))
