    ...
    (labels ((write-once ()
               (let ((num (handler-case
                              (et:repeat-upon-condition-decreasing-timeout
                                  ((et:eintr) timeout-var timeout)
                                (prog1
                                    (et:write fd (inc-pointer buf bytes-written) nbytes)
                                  (when (and timeout-var (zerop timeout-var))
                                    (return-from %write-n-bytes (values nil :timeout)))))
                            (et:epipe ()
                              ...
                              (et:repeat-upon-condition-decreasing-timeout
                                ...
                                (prog1
                                    (et:write fd (iobuf-start-pointer buf)
                                              (iobuf-length buf))
                                  (when (and timeout-var (zerop timeout-var))
                                    (return-from %flush-obuf (values nil :timeout)))))
                            (et:epipe ()
